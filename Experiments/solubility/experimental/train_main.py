...
from Source.trainer_solubility import GCNNTrainer
from Source.GCNN_FCNN.featurizers import featurize_sdf_with_solvent_experimental
from Source.GCNN_FCNN.model_solubility import GCNN_FCNN
...
parser.add_argument('--mode', type=str, default="regression", help='Mode of the target - regression, binary_classification or multiclass_classification')
parser.add_argument('--learning-rate', default=1e-3, type=float, help='Learning rate')
parser.add_argument('--use-maccskeys', action='store_true', help='To use MACCSKeys or not to use')
# parser.add_argument('--use-exp-values', type=list, default=['MW', 'RI', 'eps', 'BP', 'd', 'MP', 'WS', 'FP'], help='Experimental parameters to use')
parser.add_argument('--use-exp-values', nargs='+', default=['MW', 'RI', 'eps', 'BP', 'd', 'MP', 'WS', 'FP'], help='Experimental parameters to use')
...
featurize = partial(
    featurize_sdf_with_solvent_experimental,
    #mol_featurizer=ConvMolFeaturizer(), use_maccskeys=args.use_maccskeys,
    mol_featurizer=ConvMolFeaturizer(), use_maccskeys=False, use_exp_values=['MW', 'RI', 'eps', 'BP', 'MP', 'WS']
)
...
if args.test_sdf is not None:
    ...
    print(f"x_fully_connected: {sample.x_fully_connected}")
...
trainer = GCNNTrainer(
    ...
    output_folder=str(ROOT_DIR / "Models" / "solubility" / "experimental_descriptors" / args.experiment_name),
    epochs=args.epochs,
