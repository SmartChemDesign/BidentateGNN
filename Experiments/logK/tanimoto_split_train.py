...
for n_folds in [2, 3, 5, 10]:
    ...
    output_folder = ROOT_DIR / f"Models/tanimoto_split_{n_folds}fold_{mode}_{time_mark}"
    ...
    folds, [train_smiles, valid_smiles] = tanimoto_train_valid_split(smiles, 
                                       train_datasets, 
                                       n_folds, 
                                       batch_size, 
    ...
    with open(os.path.join(output_folder, "training_smiles.json"), "w") as jf:
        json.dump({i: tuple(smiles) for i, smiles in enumerate(train_smiles)}, jf)

    with open(os.path.join(output_folder, "valid_smiles.json"), "w") as jf:
        json.dump({i: tuple(smiles) for i, smiles in enumerate(valid_smiles)}, jf)